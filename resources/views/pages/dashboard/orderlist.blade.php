    ...
    <div class="all-orders">
        <div class="card">
            <div class="card-body py-3">
                ...
                <div class="filter">
                    <form id="filterForm" action="{{ route('orders.filter') }}" method="get" class="search-filter admin-search-filter">
                        @csrf
                        ...
                        {{-- <input type="text" name="search_by" value="{{ request('search_by') }}" placeholder="Search"> --}}
                                    ...
                                    @else
                                        @foreach ($data as $order)
                                            <tr>
                                                ...
                                                <td>{{ $order->company }}</td>
                                                @if (isset($companySettings) && $companySettings->company_id == env('RR_COMPANY_ID'))
                                                    <td>{{ $order->order_parent_company_name }}</td>
                                                @endif
                                                <td>
                                                    @if ($order->payStatus == 'pending')
                                                        <a href="{{ route('order.pay', ['oid' => $order->order_id]) }}" class="badge bg-dark">
                                                            Proceed to Payment
                                                        </a>
                                                    @else
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($order->order_createdAt)->format('M d, Y') }}</td>
                                                <td>
                                                    <div class="d-flex">
                                                    @if ($order->box_label != 1 && $order->device_label != 1)

                                                        <a href="{{ route('suborder.edit', $order->item_id) }}" class="badge bg-dark theme-bgcolor-btn-one p-2 me-1"><svg
                                                    ...
                                                    @endif
                                                        <a href="{{ route('order.detail', $order->item_id) }}" class="badge bg-dark theme-bgcolor-btn-one p-2 me-1"><svg
                                                            xmlns="http://www.w3.org/2000/svg" width="16"
                                                    ...
                                                    @if ($order->box_label != 1 && $order->device_label != 1)
                                                        <a href="#" class="badge bg-dark theme-bgcolor-btn-one p-2 deleteOrder" data-del="{{ $order->item_id }}"><svg
                                                            xmlns="http://www.w3.org/2000/svg" width="16"
